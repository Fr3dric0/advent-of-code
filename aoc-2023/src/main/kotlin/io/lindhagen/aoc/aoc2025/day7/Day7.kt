...
data class CardHand(
  ...
  val bid: Long,
  val type: String = getTypeWithSpecialJoker(cards),
): Comparable<CardHand> {
  ...
  companion object {
    ...
    fun getTypeWithSpecialJoker(cards: List<String>): String {
      val cardsAsSet = cards.toSet()
      if (cardsAsSet.size == 1) {
        // Means that all have the same size
        return "five_of_a_kind"
      }

      if (cardsAsSet.size == 2 && cardsAsSet.contains("J")) {
        println("$this can be converted to five_of_a_kind")
        return "five_of_a_kind"
      }

      if (cardsAsSet.size == cards.size && !cardsAsSet.contains("J")) {
        // Every card have different labels
        return "high_card"
      }
